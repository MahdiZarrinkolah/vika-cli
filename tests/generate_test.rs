...
use vika_cli::generator::writer::{write_schemas_with_options, write_api_client_with_options};
mod common;
use common::*;
...
async fn test_full_generation_workflow() {
    ...
    let schema_files = write_schemas_with_options(&output_dir, "users", &types, &zod_schemas, false, false).unwrap();
    assert!(!schema_files.is_empty());
    
    let api_files = write_api_client_with_options(&output_dir, "users", &api_functions, false, false).unwrap();
